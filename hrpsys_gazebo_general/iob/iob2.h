...
#include <hrpsys_gazebo_msgs/RobotState.h>
  ...
  public:
     ...
     * @retval FALSE otherwise  
     */
    int open_iob(std::string iob_name="");
  ...
  private:
    inline int joint_id_model2real(int id);
    inline void tick_service_command();

    typedef hrpsys_gazebo_msgs::JointCommand JointCommand;
    typedef hrpsys_gazebo_msgs::RobotState RobotState;
    
    void setJointStates(const RobotState::ConstPtr &_js);
    
    ros::NodeHandle* rosnode;
    
    ros::ServiceClient serv_command;
    ros::Publisher pub_joint_command;
    ros::Subscriber sub_robot_state;
    bool iob_synchronized;
    bool start_robothw = false;
    bool use_velocity_feedback = false;
    bool use_servo_on = false;
    
    JointCommand jointcommand;
    JointCommand initial_jointcommand;
    
    RobotState js;
    bool init_sub_flag = false;
    
    std::vector<double> command;
    std::vector<double> prev_command;
    ...
    timespec g_ts;
    long g_period_ns=1000000;
    long overwrite_g_period_ns = -1;
    ros::Time rg_ts;
    int num_of_substeps = 1;
    std::map<int, int> joint_model2real_map;
    std::vector<int>   joint_real2model_vec;
